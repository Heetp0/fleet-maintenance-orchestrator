...
guardrail_agent = LlmAgent(
    ...
    1. The estimated RUL must be a reasonable number between 0 and 200.
    2. Cross-check the estimated RUL with the anomalous sensors: if the engine is near failure (RUL < 30 cycles),
       at least one sensor must be anomalous (is_anomalous should be True). If RUL < 30 but is_anomalous is False,
       it is a discrepancy.
       
    Output a JSON block with the following keys:
    - "validation_passed": boolean (True if all checks pass, False otherwise)
    - "estimated_rul": integer (the verified RUL)
    - "failing_sensors": list of strings (the failing sensors)
    - "priority_level": string representing the priority tier:
      - RUL < 15: "CRITICAL"
      - 15 <= RUL < 30: "HIGH"
      - 30 <= RUL < 50: "MEDIUM"
      - RUL >= 50: "LOW"
    - "reasoning": string describing the validation results and why this priority level was assigned.
